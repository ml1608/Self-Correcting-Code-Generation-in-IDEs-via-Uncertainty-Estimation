...
def load_sep_probe(probe_dir: str):
    ...
    probe_json_path = os.path.join(probe_dir, "probe.json")
    ...
    if os.path.exists(probe_pkl_path):
        ...
        # Use recommended_threshold if available (for inference), otherwise fall back to threshold (semantic entropy threshold)
        ...
        # Get classifier type from JSON if available
        ...
        print(f"✅ Loaded SEP probe from pickle: {classifier_type}, threshold={threshold:.4f}")
...
# ============================================================
# Feature Extraction (SLT multi-layer)
# ============================================================
...
def adaptive_decode(
    ...
    sep_probe: Optional[Tuple] = None,  # (scaler, clf, threshold)
    ...
    for step in range(max_new_tokens):
        ...
        if sep_probe is not None:
            ...
            # Extract SLT features from current state
